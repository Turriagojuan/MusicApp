        ...
        if (uiState.isLoading) {
            ...
            Column(modifier = Modifier.fillMaxSize().padding(paddingValues).padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                uiState.progress?.let { progress ->
                    LessonItem(stringResource(id = LessonContentProvider.getLesson("2.1")!!.titleResId), progress.lesson_2_1_completed) { onLessonClick("2.1") }
                    Spacer(Modifier.height(12.dp))
                    LessonItem(stringResource(id = LessonContentProvider.getLesson("2.2")!!.titleResId), progress.lesson_2_2_completed) { onLessonClick("2.2") }
                    Spacer(Modifier.height(12.dp))
                    LessonItem(stringResource(id = LessonContentProvider.getLesson("2.3")!!.titleResId), progress.lesson_2_3_completed) { onLessonClick("2.3") }
                    Spacer(Modifier.height(12.dp))
                    LessonItem(stringResource(id = LessonContentProvider.getLesson("2.4")!!.titleResId), progress.lesson_2_4_completed) { onLessonClick("2.4") }
                }
